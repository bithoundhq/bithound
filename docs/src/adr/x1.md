...
[[notification_rules]]
id = "critical-to-telegram"                     # operator-picked slug
...
`notification_rules.id` is an operator-supplied string slug, not a
UUID — same shape as `CollectorId` and `BitcoinNodeId` elsewhere in
the config. Operators write a short, lowercase-hyphenated identifier
that shows up directly in audit logs and incident attribution. Slugs
must be unique within a single sidecar's config; cross-sidecar
